...
export type HolderProof = { accounts: AccountMeta[]; sgt: boolean; stake: boolean; sgtDiscountBps: number; stakeDiscountBps: number; minFeeBps: number };
const NONE: HolderProof = { accounts: [], sgt: false, stake: false, sgtDiscountBps: 0, stakeDiscountBps: 0, minFeeBps: 0 };
...
export async function holderProof(connection: Connection, programId: PublicKey, owner: PublicKey | null, tiers: FeeTiersView): Promise<HolderProof> {
  ...
  const out: HolderProof = { accounts: [ro(feeTiersPda)], sgt: false, stake: false, sgtDiscountBps: tiers.sgtDiscountBps, stakeDiscountBps: tiers.stakeDiscountBps, minFeeBps: tiers.minFeeBps };
  try {
    ...
    // SKR staking: enabled once the stake account layout is configured on-chain (stakeProgram + offsets).
    if (tiers.stakeDiscountBps > 0 && tiers.stakeProgram !== PublicKey.default.toBase58()) {
...
export function discountLabel(earlyBird: boolean, proof: HolderProof) {
  const parts = []; if (earlyBird) parts.push("early bird"); if (proof.sgt) parts.push("Seeker Genesis Token"); if (proof.stake) parts.push("SKR staking");
