...
export const SOURCE_LABEL: Record<SourceKind, string> = { onchain: "on-chain, anyone can recompute", store: "dApp Store catalog (off-chain), snapshot hash on-chain", thirdparty: "third-party index, snapshot hash on-chain" };
...
export const METRIC_COPY: Record<string, { title: string; unit: string; how: string; scale?: number; source: SourceKind; cumulative?: boolean }> = {
  ...
  dapps_week: { title: "New dApp Store listings this week", unit: "apps", cumulative: true, source: "store", how: "Increase in active dApp Store listings between the opening baseline and the closing snapshot of the store catalog. Listings pass Solana Mobile review, so they cannot be spammed within a week." },
