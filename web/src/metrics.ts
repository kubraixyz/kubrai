...
export function metricInfo(id: string) {
  if (id.startsWith("rev_week:")) { const slug = id.slice(9); if (!APP_NAMES[slug]) return undefined; const app = APP_NAMES[slug]; return { title: `New ${app} reviews this week`, unit: "reviews", cumulative: true, source: "store" as SourceKind, how: `Increase in ${app}'s total dApp Store reviews between the opening baseline and the closing snapshot, read directly from the Solana dApp Store API. Reviews can only be written from a Seeker device, one per device per app, so each extra review costs a phone.` }; }
