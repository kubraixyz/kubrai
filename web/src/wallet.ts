// Wallet Standard connection: works with Phantom, Solflare, Backpack, Seed Vault
// Wallet (when opened in Seeker's browser) without per-wallet adapters.
...
export function devWallet(): Session | null {
  ...
  let kp: Keypair;
  try {
    const raw = localStorage.getItem("kubrai.devwallet");
    kp = raw ? Keypair.fromSecretKey(Uint8Array.from(JSON.parse(raw))) : Keypair.generate();
    localStorage.setItem("kubrai.devwallet", JSON.stringify(Array.from(kp.secretKey)));
