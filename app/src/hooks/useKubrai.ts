...
export function useMarkets() { const { connection } = useConnection(); return useQuery({ queryKey: ["markets"], queryFn: async () => { await (catalogLoaded ??= loadMetricCatalog(APP.apiBase)); return fetchMarkets(connection); }, refetchInterval: 30_000 }); }
