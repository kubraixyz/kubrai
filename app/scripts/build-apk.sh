...
# Release-size settings: Seeker (and every phone we target) is arm64; one ABI instead of four,
# plus R8 shrinking. gradle.properties is regenerated by prebuild, so patch it every time.
GP=android/gradle.properties
sed -i 's/^reactNativeArchitectures=.*/reactNativeArchitectures=arm64-v8a/' "$GP"; grep -q '^reactNativeArchitectures=' "$GP" || echo 'reactNativeArchitectures=arm64-v8a' >> "$GP"
sed -i 's/^android.enableProguardInReleaseBuilds=.*/android.enableProguardInReleaseBuilds=true/' "$GP"; grep -q '^android.enableProguardInReleaseBuilds=' "$GP" || echo 'android.enableProguardInReleaseBuilds=true' >> "$GP"
sed -i 's/^android.enableShrinkResourcesInReleaseBuilds=.*/android.enableShrinkResourcesInReleaseBuilds=true/' "$GP"; grep -q '^android.enableShrinkResourcesInReleaseBuilds=' "$GP" || echo 'android.enableShrinkResourcesInReleaseBuilds=true' >> "$GP"
