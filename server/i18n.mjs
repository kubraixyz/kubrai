...
/** ?lang= (a link someone shared) → the kb_lang cookie (the picker) → the browser's Accept-Language → English. */
export function resolveLang(req, url) {
  const q = match(url?.searchParams.get("lang") ?? ""); if (q) return { lang: q, fromQuery: true };
  const c = String(req?.headers?.cookie ?? "").match(/(?:^|;\s*)kb_lang=([A-Za-z-]{2,10})/); const ck = c ? match(c[1]) : null; if (ck) return { lang: ck, fromQuery: false };
