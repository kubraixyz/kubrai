// Proof that a settlement row is real, for the process that pays rebates out of the treasury.
//
// settlements.jsonl is written by the crank on the app host. Before a row earns anybody a rebate, the payer asks the
// chain: the row's signature must be a successful transaction in which THIS program emitted PositionSettled for the
// same market, owner and fee, and SKR must have moved in it. A forged row has no such transaction. Bindings stay the
// app host's word, but a false binding can only redirect a share (at most 40 %) of a fee that was really paid, never
// create one.
//
...
export function disproof(row, tx, mint) {
  ...
  if (mint && !mints.has(mint)) return "SKR did not move in this transaction";
 ...
 */
export async function provenRows(conn, rows, { cacheFile, wanted, mint, log = () => {}, pauseMs = 400 }) {
  ...
  for (const r of rows) {
    ...
    const key = `${r.signature}:${r.owner}:${r.fee}`;
    ...
    for (let attempt = 1; attempt <= 4; attempt++) {
      ...
      catch { failed = true; await new Promise((res) => setTimeout(res, 1500 * attempt)); }
    }
