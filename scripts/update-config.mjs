...
const pick = (c) => ({ proposer: c.proposer, feeBps: c.feeBps, earlyBirdDiscountBps: c.earlyBirdDiscountBps, earlyBirdSecs: c.earlyBirdSecs, disputeWindowSecs: c.disputeWindowSecs, minBet: c.minBet });
...
for (const kv of process.argv.slice(2)) {
  ...
  args[k] = k === "proposer" ? new PublicKey(v) : typeof args[k] === "number" ? Number(v) : new anchor.BN(v);
}
